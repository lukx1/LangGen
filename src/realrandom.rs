...
impl RealRandom {
    fn pull_syllable(&mut self, pos:usize, len:usize, cfg: &dyn LangConfig) -> String{
        ...
        while tries < self.max_tries {
            let syllable = cfg.syllables() // Pull random syllable
                .get(self.rng.gen_range(0,cfg.syllables().len()))
                .unwrap(); // Can never be out of bounds

            if syllables::is_syllable_pos_valid(&syllable,pos,len,cfg){ // Check if it is valid
        ...
        panic!("Could not get a suitable syllable in {} tries",self.max_tries)
    }
}
...
impl RandomEngine for RealRandom {
    fn create_words(&mut self, min_len: u32, max_len: u32, count: u32, cfg: &dyn LangConfig) -> Vec<String> {
        ...
        for _nth_word in 0..count {
            ...
            for _nth_syllable in 0..chosen_length {
    ...
    fn with_config(_cfg: &dyn LangConfig) -> Result<RealRandom> where Self: Sized {
        Ok(RealRandom{
            rng: rand::thread_rng(),
            max_tries: 100
        })
